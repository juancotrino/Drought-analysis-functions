...
def NetCDFtoTIF(dst_ds, src_ds, dst_EPSG, bands = [1], no_data_value=-9999):
    """
...
def MatchRasterAlignment_V2(master, slave, dst_filename):
    """
    ***CODE FOUND HERE: https://github.com/jgomezdans/eoldas_ng_observations/blob/master/eoldas_ng_observations/eoldas_observation_helpers.py#L29
...
def SplitTIFBands(init_str ,raster_file, dst_folder, no_data_value=-9999):
    """
    CODE FOR CONVERTING NETCDF FILE INTO RASTER
    ...
    no_bands = raster.RasterCount
        
    for band in range(no_bands):
        arr = raster.GetRasterBand(band+1).ReadAsArray()
        arr = np.where(arr == noval, no_data_value, arr)  # change 0 to -10000

        raster_2 = gdal.GetDriverByName('GTiff').Create(dst_folder + init_str + '_' + str(band) + '.tif',cols,rows,1,gdal.GDT_Float32)
        raster_2.SetGeoTransform(gt)
        raster_2.SetProjection(prj)
        
        raster_2.GetRasterBand(1).SetNoDataValue(no_data_value)
        raster_2.GetRasterBand(1).Fill(noval)
        raster_2.GetRasterBand(1).WriteArray(arr)
